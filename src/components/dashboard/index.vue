<template>
  <div class="wrap py-3 px-3 relative pb-12">
    <div
      class="flex sticky top-0 left-0 nav-top justify-between w-full px-3 py-2"
    >
    ...
    <div class="flex justify-center items-center w-full gap-2 my-3 py-3 px-1">
      ...
      >
        ...
        <div class="balance flex flex-col justify-center items-center gap-1">
          ...
          <p class="text-white text-2xl font-bold ma-0 pa-0">
            ₦{{
              show
                ? new Intl.NumberFormat().format(userData.balance)
                : " xxxxxx"
            }}
            <v-icon @click="show = !show" color="white">{{
        ...
        <span
          @click="WalletSumary"
          class="absolute ser_txt text-white top-3 right-2"
          >Wallet History
          <v-icon small color="white"> mdi-arrow-right</v-icon></span
        >
      </div>
    </div>
    ...
    <p class="w-full pa-0 ma-0 text-left text-lg my-1 py-2 px-1 font-bold">
      Services
    </p>
      ...
      <div class="bg-priblue h-full">
        <v-list-item class="py-5">
          ...
          <v-list-item-content>
            ...
            <v-list-item-title class="text-white"
              >Balance: ₦{{ userData.balance }}</v-list-item-title
            >
          </v-list-item-content>
        </v-list-item>
        ...
        <v-list dense class="listss">
          ...
          <v-list-item
            :to="{ path: '/admin' }"
            v-if="userData.account == 'ADMIN'"
            link
            class="border-2 border-solid border-white mx-4 mb-3 rounded-md flex items-center gap-2"
          >
            <v-icon color="white">mdi-menu</v-icon>
            <v-list-item-title class="text-white text-lg pa-0 ma-0"
          ...
          <v-list-item
            v-for="item in items"
            :to="{ name: item.url }"
            class="mx-4 mb-2 flex items-center gap-3"
            :key="item.title"
            link
          >
    ...
    <v-bottom-navigation grow class="meto">
      <v-btn :to="{ path: '/' }">
        <span>Home</span>
